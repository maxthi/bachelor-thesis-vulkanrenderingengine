...
{
	/// @brief The vulkan interface wraps the vulkan api in an additional layer.
    /// The vulkan interface class only creates a single static object containing the pointers
    /// to the qt vulkan functions.
	class VulkanInterface
	{
	public:
		// Singleton
		static VulkanInterface Interface;

		void SetQVulkanFunctions(QVulkanFunctions* pQVulkanFunctions);
		void SetQVulkanDeviceFunctions(QVulkanDeviceFunctions* pQVulkanDeviceFunctions);
		QVulkanFunctions* GetQVulkanFunctions();
		QVulkanDeviceFunctions* GetQVulkanDeviceFunctions();

	private:
		QVulkanFunctions* m_pQVulkanFunctions;
		QVulkanDeviceFunctions* m_pQVulkanDeviceFunctions;

		// private constructor
		VulkanInterface();
	};

	inline QVulkanFunctions* VulkanInterface::GetQVulkanFunctions()
	{
		if (m_pQVulkanFunctions == nullptr) {
			err("VulkanInterface: QVulkanFunctions ptr is nullptr!");
		}
		return m_pQVulkanFunctions;
	}

	inline QVulkanDeviceFunctions* VulkanInterface::GetQVulkanDeviceFunctions()
	{
		if (m_pQVulkanDeviceFunctions == nullptr) {
			err("VulkanInterface: QVulkanDeviceFunctions ptr is nullptr!");
		}
		return m_pQVulkanDeviceFunctions;
	}
